---
title: "Phase 2a: Descriptive Profile of the Analytical Corpus"
subtitle: "Volume, platforms, sources, syndication, scandal register audit"
...
Phase 2a produces the descriptive foundation for the destination framing
study. It answers the first operative question (which destinations are
most present in media) and supports the other two by establishing basic
corpus characteristics that any downstream interpretation must respect.

Six components are produced.

1. Frequency profile per destination (absolute and normalised).
2. Platform distribution, showing whether a destination is carried
   predominantly by online portals, print, TV, or social media.
3. Source concentration, measured as share of articles contributed by the
   top 10 sources, to detect destinations whose coverage is driven by few
   outlets.
4. Syndication estimate through content hash clustering, to correct
   volume figures for duplicate copies of agency wire content.
5. Daily volume distribution, to distinguish destinations with steady
   baseline coverage from destinations with spike driven coverage.
6. Scandal register noise audit, checking whether the scandal register
   tag reflects genuine scandal content or picks up lexical noise.

Outputs feed Phase 2b (seasonality) and Phase 2c (framing model) with a
clean, well understood corpus description.
...
Share of articles contributed by the top N sources per destination.
High concentration means coverage is driven by few outlets and is
more vulnerable to outlet specific editorial patterns.
...
# 5. Syndication Estimate via Content Hash

Articles copied verbatim across portals (typically HINA agency wire)
produce multiple pairs with near identical content. The cleaned corpus
already carries a `content_hash` column from the audit step, so we use
...
# 7. Scandal Register Noise Audit

The scandal register tag may be inflated by lexically ambiguous stems
like *sud* which matches *sudjelovati*, *sudbina*, *sudac*. A manual
audit on a stratified sample quantifies the noise rate.
...
# 10. Summary for Next Phase

Phase 2a deliverables are the descriptive backbone. Phase 2b will build
on top of the enriched corpus (destinations_analytical_enriched.rds)
which now carries content_hash and cluster_size, allowing seasonality
analysis to optionally down weight syndicated content.

Key observations to carry forward: destinations with high source
concentration need volume figures interpreted with caution, destinations
with heavy syndication need their spikes cross checked against agency
wire cycles, and destinations with sparse temporal coverage need
seasonality conclusions qualified.
